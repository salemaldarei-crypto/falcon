...
function scopedUsers() {
    const u = SESSION.user;
    if (!u) return [];
    // System admin sees all users in the system
    if (u.role === 'admin') return DB.users;

    const myFarms = getUserFarmIds(u);
    if (u.role === 'farm_manager') {
        // Farm managers ONLY see employees assigned to their farms + their own account.
        // System admin accounts (role === 'admin') are strictly HIDDEN and NEVER returned.
        return DB.users.filter(x => {
            if (x.role === 'admin') return false; // Strictly hidden from non-admins
            if (x.id === u.id) return true;       // Manager sees own account
            if (x.role === 'employee') {
                const xFarms = getUserFarmIds(x);
                return xFarms.some(fid => myFarms.includes(fid));
            }
            return false;
        });
    }
...
function setupFalconForm() {
    ...
    document.getElementById('falconForm').addEventListener('submit', async e => {
        ...
        const falconName = gv('falconName');
        const ringNum    = gv('falconRingNumber');
        const fWeight    = gn('falconWeight');
        const fType      = gv('falconType');
        ...
        fsSave('falcons', falcon.id, falcon);
        ...
        const farmObj = DB.farms.find(f => f.id === farmId);
        const farmName = farmObj ? farmObj.name : '';
        const ringStr = ringNum ? ` [حجل: ${ringNum}]` : '';
        const msg = `تم تسجيل الصقر "${falconName}" (${fType}${ringStr}) بوزن ${fWeight} جم بنجاح في مزرعة ${farmName}`;
        toast(msg, 'ok', '🦅 تم إضافة الصقر بنجاح');

        // Highlight new falcon in the table
        setTimeout(() => {
            const table = document.getElementById('falconsTable');
            if (table) {
                const firstRow = table.querySelector('tbody tr');
                if (firstRow) {
                    firstRow.classList.add('highlight-glow');
...
function setupDailyForm() {
    ...
    document.getElementById('dailyForm').addEventListener('submit', async e => {
        ...
        const prevWeight = latestW(fid) ?? (f ? f.weight : null);
        const newWeight = gn('dailyWeight') || null;

        let diffSummary = '';
        if (newWeight && prevWeight !== null) {
            const diff = newWeight - prevWeight;
            if (diff > 0) diffSummary = ` (زيادة +${diff} جم عن السابق ${prevWeight} جم)`;
            else if (diff < 0) diffSummary = ` (نقصان ${diff} جم عن السابق ${prevWeight} جم)`;
            else diffSummary = ` (مطابق للوزن السابق ${prevWeight} جم)`;
        } else if (newWeight) {
            diffSummary = ` (${newWeight} جم)`;
        }
        ...
        // Background save to Firestore
        fsSave('logs', entry.id, entry);

        // Immediate rich confirmation toast
        const fName = f ? f.name : 'الصقر';
        const msgDesc = newWeight
            ? `تم تسجيل وزن الصقر "${fName}": ${newWeight} جم${diffSummary}`
            : `تم حفظ النشاط والتدريب للصقر "${fName}" بنجاح`;
        toast(msgDesc, 'ok', '⚖️ تم حفظ السجل بنجاح');
        ...
        // Highlight top log row
        setTimeout(() => {
            const firstLog = document.querySelector('#dailyLogList .log-item');
            if (firstLog) {
                firstLog.classList.add('highlight-glow');
                firstLog.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
            }
        }, 80);
    });
}
...
function resetDailyForm() {
    ...
    document.getElementById('weightDiff').innerHTML = '';
...
function showFalconPreview(fid) {
    ...
    if (!fid || !card) { if(card) card.style.display = 'none'; return; }
    const f = DB.falcons.find(x => x.id === fid);
    ...
    const farm = DB.farms.find(x => x.id === f.farmId);
    const lastW = latestW(f.id) ?? f.weight;

    document.getElementById('fpcName').textContent = f.name;
    const farmEl = document.getElementById('fpcFarm');
    if (farmEl) farmEl.textContent = farm ? `🌿 ${farm.name}` : '🌿 عامة';

    const ringEl = document.getElementById('fpcRing');
    if (ringEl) {
        ringEl.textContent = f.ringNumber ? `💍 ${f.ringNumber}` : '';
        ringEl.style.display = f.ringNumber ? 'inline-block' : 'none';
    }

    const lastWEl = document.getElementById('fpcLastWeight');
    if (lastWEl) lastWEl.textContent = `${lastW} ${t('unit.g')}`;

    const typeEl = document.getElementById('fpcType');
    if (typeEl) typeEl.textContent = f.type || '—';

    const ageEl = document.getElementById('fpcAge');
    if (ageEl) ageEl.textContent = fmtAge(f.birthdate);
...
function updateWeightHint() {
    ...
    if (!el) return;
    if (!fid || !newW) { el.innerHTML = ''; return; }
    const lw   = latestW(fid);
    const f    = DB.falcons.find(x => x.id === fid);
    const base = lw !== null ? lw : (f?.weight || null);
    if (!base) { el.innerHTML = ''; return; }
    const d = newW - base;
    if (d > 0) {
        el.innerHTML = `<span class="diff-badge diff-up">▲ زيادة +${d} ${t('unit.g')} عن آخر وزن (${base} ${t('unit.g')})</span>`;
    } else if (d < 0) {
        el.innerHTML = `<span class="diff-badge diff-down">▼ نقصان ${d} ${t('unit.g')} عن آخر وزن (${base} ${t('unit.g')})</span>`;
    } else {
        el.innerHTML = `<span class="diff-badge diff-same">= مطابق تماماً لآخر وزن مسجل (${base} ${t('unit.g')})</span>`;
    }
}
...
function openEditUser(id) {
    ...
    // Security boundary: non-admin can NEVER edit an admin account
    if (u.role === 'admin' && SESSION.user.role !== 'admin') {
        toast('غير مصرح لك بتعديل حساب مدير النظام', 'err');
        return;
    }
    // Farm managers can ONLY edit employees of their farms
    if (SESSION.user.role === 'farm_manager' && u.role !== 'employee' && u.id !== SESSION.user.id) {
        toast('يمكنك فقط تعديل حسابات موظفي مزرعتك', 'err');
        return;
    }
...
function renderUsersTable() {
    ...
    visible.forEach((u, i) => {
        ...
        // Edit permission:
        // Admin can edit all non-admin or self
        // Farm manager can ONLY edit employees
        ...
        // Farm manager can ONLY change their own password or their employees' passwords (NEVER admin)
        // Employee can only change their own password
        const canChangePwd = (SESSION.user.role === 'admin') || isMe || (SESSION.user.role === 'farm_manager' && u.role === 'employee');

        // Delete permission:
        // Cannot delete oneself
        // Cannot delete any admin account
        const canDel = !isMe && u.role !== 'admin' && (
        ...
        tr.innerHTML = `
            ...
            <td>
                <div class="row-acts">
                    ...
                    ${canChangePwd ? `<button class="act-btn pwd" title="🔑" onclick="openPwd('${u.id}')">🔑</button>` : ''}
...
async function deleteUser(id) {
    ...
    if (u.role === 'admin' && SESSION.user.role !== 'admin') {
        toast('لا يمكن حذف حساب مدير النظام', 'err');
        return;
    }
    if (u.id === 'u_admin') {
        toast('لا يمكن حذف الحساب الرئيسي لمدير النظام', 'err');
        return;
    }
...
function setupPwdModal() {
    ...
    document.getElementById('pwdForm').addEventListener('submit', async e=>{
        ...
        // Security boundary: non-admin CANNOT change admin's password!
        if (u.role === 'admin' && SESSION.user.role !== 'admin') {
            toast('غير مصرح لك بتغيير كلمة مرور مدير النظام', 'err');
            closeModal('pwdModal');
            return;
        }
        if (SESSION.user.role === 'farm_manager' && u.id !== SESSION.user.id && u.role !== 'employee') {
            toast('يمكنك فقط تغيير كلمات مرور موظفي مزرعتك', 'err');
            closeModal('pwdModal');
            return;
        }
...
function openPwd(id) {
    const targetUser = DB.users.find(x => x.id === id);
    if (!targetUser) return;

    // Strict security check: non-admins cannot change admin password
    if (targetUser.role === 'admin' && SESSION.user.role !== 'admin') {
        toast('غير مصرح لك بتغيير كلمة مرور مدير النظام', 'err');
        return;
    }
    if (SESSION.user.role === 'farm_manager' && targetUser.id !== SESSION.user.id && targetUser.role !== 'employee') {
        toast('يمكنك فقط تغيير كلمات مرور موظفي مزرعتك', 'err');
        return;
    }
...
function toast(msg, type='info', title='') {
    ...
    const iconSpan = `<span class="toast-icon">${icons[type] || '🔔'}</span>`;
    const titleHtml = title ? `<strong class="toast-title">${esc(title)}</strong>` : '';
    const descHtml  = `<div class="toast-desc">${esc(msg)}</div>`;
